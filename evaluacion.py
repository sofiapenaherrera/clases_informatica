...
#a) El tipo de dato de la variable nombre es str (cadena de texto), de la variable edad es int (entero), de la variable promedio es float (número decimal) y de la variable cursos es una lista conformada por cadenas de texto.
...
#<class 'list'>
#6
#c) La función len(nombre) devuelve la longitud de la cadena de texto almacenada en la variable nombre, es decir, el número de caracteres que tiene la palabra "Lucía", incluyendo el espacio y los acentos. En este caso, len(nombre) devolvería 6 porque "Lucía" tiene 6 caracteres (L, u, c, í, a).  
...
#b) La corrección permite obtener resultados válidos porque se convierte la entrada del usuario a tipo float, lo que permite realizar las operaciones matemáticas correctamente. Además, al imprimir el área y el costo, se convierte el resultado a cadena de texto utilizando str(), lo que permite unir la información sin errores.
